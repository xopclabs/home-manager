...
let
  ...
  floorpConfigPath = if isDarwin then
    "Library/Application Support/Floorp"
  else
    ".floorp";

  profilesPath =
    if isDarwin then "${floorpConfigPath}/Profiles" else floorpConfigPath;
        ...
        type = types.attrsOf (types.submodule ({ config, name, ... }: {
          options = {
            ...
            settings = mkOption {
              type = types.attrsOf (jsonFormat.type // {
                description =
                  "Floorp preference (int, bool, string, and also attrs, list, float as a JSON string)";
              });
              default = { };
              example = literalExpression ''
                {
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      in {
        ...
        message = "Must have exactly one default Floorp profile but found "
          ...
          || profile.search.order != [ ] || profile.search.engines != { }) {
            ...
            source = let
              ...
              salt = if profile.search.default != null then
                profile.path + profile.search.default + disclaimer "Floorp"
              else
                null;
